...
const Answer1 = () => {
  ...
  const SubmitAns = async (AnsId, QueId, index) => {
    try {
      setLoading(true);
      const res = await axios.post(
        "/add_question",
      ...
      if (res.data.status == "1") {
        if (Quetions.data.length == index + 1) {
          Navigate("/AllAns");
        } else {
        ...
        if (
          res.data.message ==
          "You have already answered this question in this game."
        ) {
          Navigate("/AllAns");
        } else {
          Navigate("/Juegos/all");
  ...
  return (
    <div>
      ...
      <AppBar />
      <Title
        title={`Pregunta cierra el 27 de Junio del 2022 - ${
          Quetions.end_time ? Quetions.end_time : ""
        }`}
      />
      <div
                        ...
                        {item.answers.map((_item, _index) => {
                          return (
                            <Button
                              ...
                              style={{ fontSize: "25px" }}
                              onClick={() =>
                                SubmitAns(_item.id, item.id, index)
                              }
                            >
                              {_item.answer}
                            </Button>
